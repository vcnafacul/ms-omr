...
class OmrEngineError(Exception):
    """Falha ao executar o OMRChecker ou ao interpretar sua saída."""
...
def run_omr(image: bytes | str | Path, template_dir: str | Path) -> RawOmrResult:
    ...
    with tempfile.TemporaryDirectory(prefix="omr-") as tmp:
        ...
        try:
            ...
            raise OmrEngineError(f"OMRChecker excedeu {_TIMEOUT_SECONDS}s") from exc
...
def _parse_results(output_dir: Path, image_name: str) -> RawOmrResult:
    ...
    if not csvs:
        raise OmrEngineError("OMRChecker não gerou CSV de Results")

    row = _find_row(csvs, image_name)
    if row is None:
        raise OmrEngineError(f"leitura de '{image_name}' ausente no CSV de Results")
